...
export default function LoginPage() {
  ...
  const initializeGoogle = useCallback(() => {
    ...
    if (clientId && window.google?.accounts && !googleInitializedRef.current && !window.__googleInitialized) {
    ...
    <div className="min-h-screen bg-cream-100 dark:bg-[#1A1A1A] flex items-center justify-center px-4 py-12 relative">
      ...
      <div className="w-full max-w-md relative z-10">
        ...
        <div className="text-center mb-10">
          ...
          <h1 className="font-display text-3xl font-bold text-ink dark:text-cream-100 tracking-tight">Welcome back</h1>
          <p className="text-ink-400 dark:text-white/40 mt-2.5 text-[15px]">Sign in to access your bookmarks</p>
        </div>
        ...
        <div className="bg-white dark:bg-[#242424] p-8 rounded-2xl border border-cream-300 dark:border-white/10 shadow-soft">
            ...
            >
              {googleLoading ? (
                <button disabled className="w-full social-btn opacity-70">
                  <div className="w-5 h-5 border-2 border-cream-400 border-t-terra-500 rounded-full animate-spin"></div>
                ...
                <button type="button" disabled className="w-full social-btn opacity-50 cursor-not-allowed">
                  ...
                  <span className="text-ink-400 dark:text-white/40">Google Sign-In Not Configured</span>
                </button>
              ) : null}
            </div>
          </div>
          ...
          <div className="relative mb-6">
            ...
            <div className="relative flex justify-center text-sm">
              <span className="px-4 bg-white dark:bg-[#242424] text-ink-400 dark:text-white/40">or continue with email</span>
            </div>
          </div>
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div className="mb-6">
              <div className="flex items-center justify-between mb-2">
                <label htmlFor="password" className="block text-sm font-medium text-ink-600 dark:text-white/70">
                  Password
                </label>
                <Link href="/forgot-password" className="text-sm text-terra-500 hover:text-terra-600 font-medium">
                  Forgot password?
                </Link>
              </div>
          ...
          <p className="mt-6 text-center text-ink-500 dark:text-white/50 text-[15px]">
            ...
            <Link href="/register" className="text-terra-500 hover:text-terra-600 font-semibold">
              Sign up free
            </Link>
          </p>
        </div>
        ...
        {/* Trust badges */}
        <div className="mt-10 flex items-center justify-center gap-8 text-sm text-ink-400 dark:text-white/40">
